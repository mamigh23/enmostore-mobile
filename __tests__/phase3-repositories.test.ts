...
describe('Phase 3 repositories', () => {
  test('maps valid bootstrap', async () => {
    ...
    await expect(new BootstrapRepository(request).getBootstrap()).resolves.toEqual({
      supportedLocales: ['tr', 'en', 'de', 'ja'],
      currency: 'EUR',
    });
    expect(request).toHaveBeenCalledWith('/api/v1/bootstrap', { method: 'GET' });
  });
  ...
  test.each([
    ...
    await expect(new BootstrapRepository(request).getBootstrap()).rejects.toMatchObject({
  ...
  test('maps product list and pagination', async () => {
    ...
    const result = await new ProductRepository(request).getProducts({ page: 1 });
  ...
  test('maps product detail', async () => {
    ...
    await expect(new ProductRepository(request).getProduct(1)).resolves.toEqual(detail);
    expect(request).toHaveBeenCalledWith('/api/v1/products/1', { method: 'GET' });
  });
  ...
  test.each([
    ...
    await expect(new ProductRepository(request).getProducts()).rejects.toMatchObject({
      code: 'invalid_response',
    });
  });
  ...
  test.each([
    ...
    { sizes: [1] },
    { colors: [false] },
  ])('rejects malformed detail arrays', async change => {
    const detail = {
      id: 1,
      name: 'Perfume',
      description: 'Description',
    ...
    const { request } = requester({ data: detail });
    await expect(new ProductRepository(request).getProduct(1)).rejects.toMatchObject({
      code: 'invalid_response',
    });
  });
  ...
  test('maps root and child categories', async () => {
    const categories = [
      ...
      { id: 2, name: 'Child', parentId: 1, imageUrl: 'https://example.com/c.jpg' },
    ];
    const { request } = requester({ data: categories });
    await expect(new CategoryRepository(request).getCategories()).resolves.toEqual(categories);
    expect(request).toHaveBeenCalledWith('/api/v1/categories', { method: 'GET' });
  });
  ...
  test('rejects malformed category parentId', async () => {
    ...
    await expect(new CategoryRepository(request).getCategories()).rejects.toMatchObject({
      code: 'invalid_response',
    });
  });
  ...
  test('rejects empty search before making a request', async () => {
    const { request } = requester({ data: [], meta: { ...meta, total: 0, totalPages: 0 } });
    await expect(new SearchRepository(request).searchProducts('   ')).rejects.toBeInstanceOf(
      ApiError,
    );
